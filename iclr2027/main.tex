...
\begin{enumerate}
    ...
    \item \textbf{Differential Repairability:} We show that the trajectory-based taxonomy identifies errors that are differentially amenable to correction: under identical steering, preservation failures recover at 3.7$\times$ the rate of formation failures, and selective intervention guided by the predictor improves per-intervention efficiency by 127\%. We frame this as an offline oracle-assisted validation of the taxonomy's practical relevance, not a deployable inference-time method.
    \item \textbf{Benchmark Resource:} We release \texttt{InfoDyn-Bench}, a benchmark of layered information trajectories, derived dynamics metrics, and failure labels across six models and seven tasks, to support process-level LLM error analysis.
\end{enumerate}
...
\paragraph{Extension to multi-token answers.}
When the gold answer $y^\star$ consists of multiple tokens $(y^\star_1, y^\star_2, \ldots, y^\star_m)$, we define the \emph{answer-level rank} at layer $\ell$ as the best (minimum) rank achieved by any answer token:
\begin{equation}
r^{\mathrm{multi}}_\ell = \min_{1 \le i \le m} r_\ell(y^\star_i) .
\end{equation}
All answer-level analyses (Table~\ref{tab:taskdecay}, signal-loss rates) use $r^{\mathrm{multi}}_\ell$ in place of $r_\ell(y^\star)$. This ensures that multi-word entities (e.g., ``New York'') and multi-digit numbers are detected as competitive if \emph{any} constituent token enters the top-$k$ region. The competitive-margin trajectory $\mathrm{CIS}^{\mathrm{comp}}_\ell$ and the strict competitive-decay label (Eq.~8, used in Figure~\ref{fig:taxonomy}) continue to use the first answer token $y^\star_1$, which provides the most conservative estimate.

The minimum-over-tokens rule is permissive, so we validate the taxonomy under a stricter teacher-forced, sequence-level protocol. We append the gold answer to the prompt (teacher forcing) and, for each answer token, read the hidden state at the position that predicts it, decoding the gold rank and log-probability at every layer. We then aggregate across answer positions with three well-defined rules applied consistently at every layer: \textbf{ALL} (every answer token is top-$k$ at the same intermediate layer), \textbf{MEAN} (the mean per-token rank is top-$k$), and \textbf{JOINT} (the intermediate sequence log-likelihood exceeds the final one). On all 1{,}494 Qwen3-8B errors, the sequence-level preservation rate is 13.2\% (ALL), 25.0\% (MEAN), and 87.8\% (JOINT), whereas the permissive minimum-over-tokens rule gives 90.6\%. The inflated minimum-rule rate confirms that ``any token ever entering top-$k$'' is a cherry-picking artifact: under ALL the rate decreases monotonically with answer length (24.3\% for single-token, 13.5\% for two-token, and 11.9\% for answers of three or more tokens), exactly the length bias that the permissive criterion introduces, and the per-rule comparison on long-answer tasks is dramatic (long-context comprehension: 0.5\% under ALL vs 27.6\% under the minimum rule). We therefore report the conservative first-token and ALL-based rates throughout and treat the minimum rule only as a permissive upper bound.
...
We report bootstrap 95\% confidence intervals for key proportions where sample size permits, and we report per-model and per-task numbers alongside aggregates.
...
\caption{Measurement bias in intermediate state decoding. Left: observed pairwise crossing rate (92.2\%) is lower than both the random-competitor null (98.8\%) and the rank-matched null (98.7\%), showing that crossing is a trivially weak criterion. Right: the uncalibrated raw logit lens inflates intermediate peak margins by ~2.0$\times$ relative to the tuned lens. All left-panel numbers are read directly from the matched-null analysis output.}
\label{fig:bias}
\end{figure}
...
Applying our strict criterion (Eq.~8: rank $\le 5$, $\mathrm{CIS}^{\mathrm{comp}}_\ell > 0$, and final $\mathrm{CIS}^{\mathrm{comp}}_L < 0$) to error examples reveals that preservation failures under this definition are a small minority. On the Qwen3-8B calibrated analysis subset, the tuned lens identifies 8.9\% (bootstrap 95\% CI [6.4\%, 11.5\%]) under the strict criterion. Applying the same strict criterion to the full seven-task error set (1{,}494 errors) gives a consistent overall rate of 10.0\% (95\% CI [8.4\%, 11.4\%]), with clear task structure: multi-hop QA 14.2\% [9.5, 19.5], world knowledge 12.6\% [7.8, 18.0], long-context comprehension 12.1\% [8.0, 16.6], knowledge QA 12.0\% [8.5, 15.9], science reasoning 10.9\% [7.0, 14.8], commonsense 9.2\% [5.6, 13.3], and mathematical reasoning 0.0\% [0.0, 0.0]. To keep the strict and permissive metrics strictly separated, Table~\ref{tab:taskdecay} reports \emph{both} per task: the strict teacher-forced ALL-based sequence-level rate as the main metric and the broader signal-loss rate (any answer token entering top-5 then falling below rank 10) only as a permissive upper bound. The strict criterion (Eq.~8) requires the gold signal to lead the eventual competitor, while the signal-loss metric only requires intermediate competitiveness followed by final-layer decline. The two strict criteria also diverge informatively on long-answer tasks: Eq.~8 scores the \emph{first} answer token (long-context comprehension 12.1\%), whereas the sequence-level ALL rule requires every token simultaneously (0.5\%), showing that formation on long answers is fragmentary: first tokens form, full sequences do not. We base all conclusions on the strict criteria and use the permissive rates solely as a sensitivity bound.

This rate is not a probe artifact: the tuned-lens translators are trained strictly on unlabelled prompts by predicting the model's own final-layer logits, without observing ground-truth answers or correctness labels, so no downstream task information leaks into intermediate readouts (Appendix~\ref{app:probe}).
...
Table~\ref{tab:taskdecay} reports the per-task preservation rate under the strict teacher-forced sequence-level criterion (ALL: every gold-answer token enters the top-$k$ at the same intermediate layer under teacher forcing; Section 3.3), which is our main answer-level metric, alongside the permissive minimum-over-tokens rate as a sensitivity upper bound. Under the strict criterion, multi-hop QA (24.2\%) and knowledge QA (23.0\%) show the highest rates, world knowledge and science reasoning are intermediate (16.8\%, 12.2\%), and mathematical reasoning (4.3\%) and long-context comprehension (0.5\%) are lowest. The comparison between the two metrics is itself informative: long-context comprehension ranks second-highest under the permissive rule (27.6\%) but drops to 0.5\% under the strict rule, because its answers span many tokens and the permissive ``any token ever top-5'' criterion is trivially easy to satisfy with long answers, so the formation that does occur is fragmentary, covering individual tokens rather than the full sequence (the strict rate decreases monotonically with answer length: 24.3\% for single-token, 13.5\% for two-token, 11.9\% for longer answers). Preservation failures with \emph{sequence-level} integrity are therefore concentrated in short-answer factual tasks, consistent with our interpretation that the model forms a correct factual association but fails to maintain it through late-layer computation, whereas procedural computation either succeeds outright or never forms the final-answer signal.

\begin{table}[t]
\centering
\caption{Preservation rate by task on Qwen3-8B (1{,}494 errors). Main metric: strict teacher-forced sequence-level criterion (ALL, i.e.\ every gold-answer token simultaneously top-5 at some intermediate layer; overall 13.2\%). Upper bound: permissive min-over-tokens rule (any gold token ever entering top-5 then falling below rank 10; overall 90.6\%), which inflates rates with answer length (Section 3.3). Tasks ordered by the strict rate.}
\label{tab:taskdecay}
\begin{tabular}{lcccc}
\toprule
...
MMLU & World knowledge (multi-subject) & 167 & 16.8\% & 23.4\% \\
ARC-Challenge & Science reasoning & 229 & 12.2\% & 23.6\% \\
CommonsenseQA & Commonsense reasoning & 196 & 9.7\% & 12.2\% \\
GSM8K & Math reasoning & 230 & 4.3\% & 5.2\% \\
QuALITY & Long-context comprehension & 199 & 0.5\% & 27.6\% \\
\midrule
...
The low GSM8K rate is not an artifact of single-token tracking: computing the best rank across all gold-answer tokens reproduces the same rate, and in multi-step reasoning incorrect final answers stem from intermediate reasoning errors rather than a formed-then-decayed final-answer signal (Appendix~\ref{app:thresholds}).

The taxonomy is stable under the competitiveness threshold (4.8\% at $k{=}1$, 8.5\% at $k{=}5$, 10.1\% at $k{=}10$, 16.9\% at $k{=}50$) and under the choice of lens, which changes absolute margins but not the relative ordering between correct and incorrect predictions (Appendix~\ref{app:thresholds},~\ref{app:probe}).
...
As shown in Figure~\ref{fig:prediction}a, the full-trajectory model achieves a 5-fold cross-validated AUC of $0.75 \pm 0.02$ (95\% CI [0.70, 0.80]) on Qwen3-8B within-task decay prediction, substantially outperforming single-layer midpoint snapshot baselines. Common midpoint confidence proxies perform poorly: the gold margin $\mathrm{CIS}_{t_0}$ yields AUC 0.39, midpoint rank yields 0.51, and midpoint output entropy reaches 0.61. Simple summary features such as margin-plus-slope (AUC 0.61) and running maximum (0.61) also fail to capture the full trajectory signal. This demonstrates that historical trajectory evolution adds substantial predictive value beyond any current-state snapshot. We use L2-regularized logistic regression (5-fold stratified CV, StandardScaler normalization, random seed 42); the out-of-fold average precision (PR-AUC) is 0.71, broadly concordant with the ROC-AUC because the decay label is not rare (positive in 46\% of examples). To avoid ambiguity, we fix a single headline number throughout: the linear-classifier 5-fold cross-validated AUC $0.75 \pm 0.02$; a single held-out split gives a consistent 0.79 (within the CV confidence interval), and a nonlinear random forest reaches $0.92 \pm 0.01$ (permutation test $p < 0.02$), which we report only as an upper bound on the available predictive structure. All headline claims use the conservative linear cross-validated result.

\paragraph{Predictive power exceeds final-layer confidence signals.}
We next evaluate whether final-layer output confidence signals can predict decay without intermediate trajectories. We compared the first-half trajectory predictor against baselines computed from \emph{final-layer} outputs: final gold margin (AUC 0.67), final gold rank (0.70), and final output entropy (0.68). A baseline combining all three final-layer signals reaches AUC 0.70, still below the first-half trajectory. Because the trajectory prefix observes only early hidden states while final-layer baselines observe the fully executed computation, this gap confirms that early layer history contains dynamic signals not captured by final-layer output confidence.
...
\paragraph{An inference-available predictor requiring no gold token.}
\label{sec:nonoracle}
The predictors above use the gold token to compute rank and margin trajectories, which is available offline but not at deployment time. To show that decay is predictable from quantities observable during ordinary generation, we train a predictor that uses \emph{only} the model's own top-5 token distributions at the first half of layers, without ever referencing the gold answer. Every feature was audited to be strictly prefix-only: layer-wise statistics of the top-1 identity switching rate (leader churn), the fraction of prefix layers held by the leader anchored at the \emph{mid-network} top-1 (never the final-layer output), the margin between the first and second candidates, and the entropy of the top-5 mass. A random forest on these features predicts preservation failures with pooled out-of-fold ROC-AUC 0.669 (bootstrap 95\% CI [0.630, 0.709]; PR-AUC 0.383 vs a 0.294 base rate), with leader churn as the most informative feature; flagging the top 20\% of errors by predicted probability yields 1.35$\times$ precision lift over random selection. In deployment the trigger is calibrated once on a pool of labeled errors covering the target task distribution, precisely the setting measured by the pooled out-of-fold AUC of 0.669, after which it applies to new inputs without any gold information.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig5_prediction.pdf}
\caption{Decay prediction results. (a) Within-task AUC comparing the full trajectory against snapshot and final-layer baselines (bars show the single held-out split; the headline number is the 5-fold cross-validated $0.75 \pm 0.02$). (b) Cross-task transfer matrix. (c) Cross-model transfer matrix.}
\label{fig:prediction}
\end{figure}

\section{Behavioral Intervention and Component Analysis}

We first establish that the gold direction is behaviorally necessary: ablating it from late-layer hidden states flips 40--87\% of correct examples to wrong across all six models (Qwen3-14B/8B/4B, Qwen2.5-7B, Llama-3.1-8B, Mistral-7B), while random-token controls flip almost none, and the specificity survives a norm-matched protocol in which every control direction removes the same amount of hidden state (Appendix~\ref{app:ablation}). We then locate which component suppresses the margin and validate the suppression causally.
...
The final-layer MLP (layer 35 for Qwen3-8B) has the largest negative direct attribution among all layers. The peak-state restoration experiment in Section~\ref{sec:causal-patch} establishes causally that restoring the decayed gold-direction component recovers the gold answer for preservation failures, and component-selective patching in the same section confirms the suppression is direction-specific and carried by the late-layer MLPs collectively, with layer 35 as the largest single direct contributor. Figure~\ref{fig:intervention}b shows the per-layer attribution.

\paragraph{Component-level attribution: which component contributes most negatively to the gold-vs-competitor margin?}
\label{sec:components}
To localize where the decay is concentrated, we decomposed the margin change at each layer into its attention and MLP components. We run a forward pass and read off the margin before attention, after attention, and after the MLP at every layer, so that the drop attributable to attention versus the MLP is separated per layer. Across incorrect examples, the total negative contribution of the MLP is larger than that of attention (cumulative $-4.71$ vs $-3.42$), and the dominant single source is the layer-35 MLP ($-3.42$ of the total). This is the same component that direct logit attribution flags as the largest negative contributor, so two independent attribution methods agree: for incorrect examples, the suppression of the gold signal is concentrated in the \emph{late-layer MLPs}, with layer 35 the largest single direct contributor. By contrast, correct examples show no dominant negative component (attention $-0.52$ vs MLP $-0.51$, both near zero). The decay that defines a preservation failure is therefore not spread uniformly across the network but is driven disproportionately by the late-layer MLP; the component-selective patching experiment in Section~\ref{sec:causal-patch} confirms this suppression is causal and direction-specific, and further shows it is distributed across the final MLPs (layers 31--35) rather than carried by layer 35 alone.
...
We validate the taxonomy's repairability through a sequence of increasingly stringent controls, each answering the objection raised by the previous: (i) calibrated steering shows preservation failures recover far more than formation failures; (ii) peak-state restoration reproduces this while intervening on the intermediate state, decoupling it from final rank; (iii) rank-matching removes the final-rank confound by construction; (iv) gold-direction patching establishes causality against an orthogonalized null; and (v) component-selective patching localizes the suppression. We report each in turn.
...
\paragraph{Representation-engineering steering validates differential repairability.}
To further validate that the formation-preservation distinction corresponds to different intervention regimes, we construct an activation-steering pipeline. The steering direction is computed as the mean hidden-state difference between correct and incorrect examples at the last four layers (a standard Representation Engineering contrast). The intervention trigger is the trajectory predictor. We note that this pipeline requires access to the gold-answer trajectory at analysis time and is therefore an offline oracle-assisted validation rather than a deployable inference-time method; its purpose is to confirm that the taxonomy identifies differentially repairable errors. Under this pipeline, selective steering (applied only to predicted preservation failures) achieves 9.1\% recovery per intervention versus 4.0\% for steering all errors indiscriminately, a 127\% efficiency gain while saving 72\% of the intervention budget. The ground-truth breakdown confirms the distinction: preservation failures recover at 6.6\% while formation failures recover at only 1.8\% (3.7$\times$ difference) under the same steering strength.

\paragraph{Closing the loop: driving the intervention with the non-oracle trigger.}
The 127\% figure above uses the oracle-assisted trajectory predictor as the trigger. We repeated the closed loop with the strictly non-oracle, leak-audited predictor of Section~\ref{sec:nonoracle} as the trigger: the predictor is calibrated per task on labeled errors (within-task out-of-fold probabilities, so no sample is scored by a model trained on itself), the steering direction is estimated from task-stratified calibration samples, and evaluation uses a task-balanced error set (30 errors from each of the seven tasks). At 20\% coverage, selective steering recovers 9.5\% per intervention versus 5.7\% for indiscriminate steering, a 67\% efficiency gain while spending only 20\% of the intervention budget, and the trigger flags true preservation failures at 64\% precision against a 33\% base rate (1.9$\times$ lift). The gain is stable at 10\% and 20\% coverage. Under the same steering, ground-truth preservation failures recover at 8.6\% versus 4.3\% for formation failures (2.0$\times$), reproducing the differential-repairability signature without any gold-conditioned feature at test time. The non-oracle trigger thus retains the budget savings, the precision lift, and about half the efficiency gain of the oracle-assisted trigger.

\paragraph{Peak-state restoration decouples repairability from final rank.}
To rule out that preservation failures recover merely because their gold token sits closer to the top of the \emph{final-layer} distribution (which would make the effect an artifact of final rank rather than a distinct internal mechanism), we intervene on the \emph{intermediate} state instead of the output. For each error we identify the layer $L^\star$ where the gold rank peaked, capture the last-token residual at $L^\star$, and for every subsequent layer blend the current residual toward that captured peak state, $h_\ell' = (1-\alpha)\,h_\ell + \alpha\,h_{L^\star}$, so that the signal is held near its peak rather than allowed to decay. This intervention is driven entirely by the internal trajectory and never references the final-layer distribution. At $\alpha = 0.8$, preservation failures recover at 8.3\% while formation failures recover at only 3.3\% (2.5$\times$); at $\alpha = 0.5$ the effect is preservation-exclusive (5.0\% vs 0.0\%). Because preservation failures possess a strong peak state to restore while formation failures do not, the separation persists even though the intervention is blind to final rank, confirming that differential repairability reflects the intermediate mechanism rather than the output-layer distance.

\paragraph{Rank-matched intervention.}
To control the final-rank confound directly, we match each preservation failure to a \emph{true} formation failure (peak gold rank $> 20$, i.e.\ never within the top-20 candidate set at any layer, well outside the $k=5$ competitiveness band) with the same final gold rank (within $\pm 3$), and apply the peak-state restoration above to both. The matched groups have nearly identical final-rank distributions (median 8 for preservation vs 11 for formation) but sharply different peak states (median peak rank 3 vs 31), isolating exactly the quantity of interest: the two failures are equally far from the top at the output, yet one had a strong intermediate signal and the other never did. At full restoration strength ($\alpha = 1$), peak-state restoration recovers 12.3\% of preservation failures versus 1.8\% of their rank-matched formation counterparts (7.0$\times$; 57 matched pairs on Qwen3-8B; paired bootstrap 95\% CI on the rate difference $[1.8\%, 19.3\%]$, excluding zero; McNemar exact $p = 0.070$). The effect strengthens monotonically with restoration strength ($3.0\times$ at $\alpha = 0.8$, $6.0\times$ at $\alpha = 0.9$, $7.0\times$ at $\alpha = 1$) while the formation rate stays flat, and an output-logit bonus applied to the same matched pairs produces no separation because it exploits precisely the final rank that has been equalized. As the design is paired and the sample size modest, we treat this as directionally consistent evidence; the activation-patching experiment below provides the statistically decisive causal test.

\paragraph{Causal activation patching.}
\label{sec:causal-patch}
The layer-35 MLP attribution in Section~\ref{sec:components} is correlational. We first test whether the \emph{decayed gold-direction signal itself} is causal, then run a component-selective test for the MLP. Peak-state patch: at every layer after $L^\star$, we restore the gold-direction component of the residual that decayed relative to the peak state, $h_\ell' = h_\ell + \beta\,\mathrm{clamp}_{\ge 0}\!\big((h_{L^\star}-h_\ell)^\top \hat{w}_{y^\star}\big)\,\hat{w}_{y^\star}$, where $\hat{w}_{y^\star}$ is the unit unembedding direction of the gold token. We report the \emph{net} recovery rate: the fraction of errors that were \emph{not} already top-1 at baseline but become top-1 after the patch, so that the metric cannot be inflated by preservation failures whose gold token was already near the top. We compare against two controls under the identical patch: formation failures, and a matched-null control that restores the decayed component along a random competitor direction \emph{orthogonalized against the gold direction} (same peak state, same mechanism, same strength, but guaranteed to carry no gold-direction signal). On Qwen3-8B ($n=300$, $\beta=4$), the gold patch recovers 7.3\% of preservation failures versus 0.7\% of formation failures (10.9$\times$; Fisher exact $p=0.005$) and 0.0\% of the matched-null control (Fisher exact $p=0.003$). The null control recovering exactly zero (0/109) shows that the effect is specific to the gold direction rather than to any large restoration. This establishes that the decayed gold-direction signal is causal: restoring it reinstates the answer for preservation failures while doing nothing for formation failures that never carried the signal.

\paragraph{Component-selective patching: causal, direction-specific, and distributed suppression.}
The peak-state patch above restores the signal at every layer after $L^\star$ and therefore does not, by itself, identify \emph{which component} removed it. We thus patch a \emph{single component at a single layer}: at the MLP (or attention) output of layer $\ell$, we delete exactly the margin-suppressive part of the component's write, $m' = m - \gamma\,\mathrm{clamp}_{\le 0}(m^\top \hat{d})\,\hat{d}$, where $\hat{d}$ is the unit gold-vs-competitor contrast direction, the same quantity the attribution decomposes. Conditions differ only in the target: the layer-35 MLP, adjacent MLPs (31--34), the layer-35 attention, a random direction orthogonalized against $\hat{d}$, and a classic full zero-ablation. On preservation failures ($n{=}150$; net recovery among the 109 not already top-1), removing the suppressive component of the layer-35 MLP alone recovers 11.0\%, 20.2\%, and 31.2\% of errors at $\gamma = 1, 2, 4$, while the random-direction control recovers exactly 0/109 at every strength (Fisher $p < 0.001$): the effect is specific to the margin direction rather than to perturbing the component. The same patch applied to adjacent MLPs recovers similarly (layer 34: 12.8--33.0\%; layer 33: 11.9--25.7\%; layer 31: 4.6--22.0\%), with no significant difference between layers 35 and 34 (Fisher $p \ge 0.67$); the layer-35 attention is somewhat weaker at moderate strength (11.9\% at $\gamma{=}2$, $p = 0.070$ vs the MLP). The zero-ablation control shows the complementary asymmetry: deleting the \emph{entire} layer-35 MLP write recovers least (4.6\%) and \emph{worsens} the mean gold rank by 13.0, because the component also carries essential non-suppressive computation. Together these controls establish two facts. First, the late-layer suppression is causal and direction-selective: it can be removed from a single component's write and the gold answer returns, while removing a random direction does nothing. Second, the suppression is \emph{distributed} across the final MLPs (layers 31--35) rather than unique to layer 35: the DLA ranking reflects direct-effect magnitude, not unique causal necessity. We therefore attribute the decay to the late-layer MLP population, with layer 35 as its largest single direct contributor.

\section{InfoDyn-Bench}

We release \texttt{InfoDyn-Bench}, a benchmark of layered information trajectories for LLM error analysis. Unlike prior resources that store only final-layer outputs or static activations, \texttt{InfoDyn-Bench} records, for each example, the full per-layer trajectory of the gold answer alongside derived failure labels. Table~\ref{tab:benchmark} (Appendix~\ref{app:bench}) summarizes its contents. It covers six models across three families and seven tasks, and each example carries the raw and tuned-lens trajectories, a set of derived dynamics metrics, a failure-pattern label, and a decay-prediction split.
...
Formation failures may need retrieval, reasoning correction, or knowledge editing \citep{meng2022rome}. Preservation failures may suit late-layer steering \citep{turner2023activation, zou2023representation}. Reasoning-state failures need tracking intermediate variables. Even though preservation failures are a minority of errors, identifying them is still informative: they are the errors where a correct-answer signal was present at an intermediate layer and subsequently lost, so they are a candidate target for interventions that retain rather than inject knowledge; Section~\ref{sec:causal-patch} provides causal and component-selective evidence for such retention-style interventions, and scaling them into a practical repair method remains future work.
...
The competitive margin is an operational proxy for output readiness, not knowledge itself, and the tuned lens may carry residual probe bias (though the separation holds under three independent decoders). Tracking the final-answer token does not fully describe multi-step reasoning such as GSM8K, and sequence-level formation on long multi-token answers is measured only conservatively. The taxonomy depends on a competitiveness threshold, though the rate changes slowly across $k \in \{1,\ldots,50\}$. Finally, our analysis spans 4B--14B models across three families; we do not claim generality to much larger or reasoning-specialized models.
...
The low preservation rate on GSM8K (5.2\%) raises the question of whether it reflects a genuine property of mathematical reasoning or merely a limitation of single-token tracking. To test this, we implemented multi-token tracking that computes the best rank across \emph{all} tokens of the gold answer at each layer (not just the first token). On GSM8K, multi-token tracking produces the same 5.2\% rate, confirming that the low preservation rate is not an artifact of single-token measurement but a real characteristic of math reasoning: most GSM8K answers are single-token numbers in the model's vocabulary, and the model either computes the final answer correctly or never forms it as a competitive candidate. More broadly, in multi-step chain-of-thought reasoning, incorrect final answers stem from intermediate reasoning errors rather than from a correct final-answer signal that later decays. Under our measurement framework, preservation failures are therefore most prevalent in factual retrieval and long-context tasks, and genuinely rare in procedural computation.
...
\section{Taxonomy Visualization}
\label{app:taxonomy}

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig4_taxonomy.pdf}
\caption{Operational error taxonomy across tasks for Qwen3-8B under the strict criterion (Eq.~8: rank $\le 5$ and $\mathrm{CIS}^{\mathrm{comp}} > 0$ at some intermediate layer, and $\mathrm{CIS}^{\mathrm{comp}}_L < 0$ at the final layer). This strict definition yields lower preservation-failure counts than the signal-loss rate in Table~\ref{tab:taskdecay}, which uses a broader criterion (any gold token entering top-5 then falling below rank 10). The two metrics capture different aspects: Eq.~8 requires the gold signal to lead the eventual competitor, while Table~\ref{tab:taskdecay} only requires intermediate competitiveness followed by final-layer decline.}
\label{fig:taxonomy}
\end{figure}
