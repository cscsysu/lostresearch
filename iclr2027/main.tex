...
A natural objection to the steering results above is that preservation failures may recover more simply because their gold token sits closer to the top of the \emph{final-layer} distribution, so any output-level nudge is more likely to succeed; the effect would then be an artifact of final rank rather than evidence of a distinct internal mechanism. We rule this out with an intervention that acts on the \emph{intermediate} state instead of the output. For each error we identify the layer $L^\star$ where the gold rank peaked, capture the last-token residual at $L^\star$, and for every subsequent layer blend the current residual toward that captured peak state, $h_\ell' = (1-\alpha)\,h_\ell + \alpha\,h_{L^\star}$, so that the signal is held near its peak rather than allowed to decay. This intervention is driven entirely by the internal trajectory and never references the final-layer distribution. At $\alpha = 0.8$, preservation failures recover at 8.3\% while formation failures recover at only 3.3\% (2.5$\times$); at $\alpha = 0.5$ the effect is preservation-exclusive (5.0\% vs 0.0\%). Because preservation failures possess a strong peak state to restore while formation failures do not, the separation persists even though the intervention is blind to final rank, confirming that differential repairability reflects the intermediate mechanism rather than the output-layer distance.

\paragraph{Rank-matched intervention.}
To control the final-rank confound directly, we match each preservation failure to a \emph{true} formation failure (peak gold rank $> 20$, so it never carried a strong intermediate signal) with the same final gold rank (within $\pm 3$), and apply the peak-state restoration above to both. The matched groups have nearly identical final-rank distributions (median 8 for preservation vs 11 for formation) but sharply different peak states (median peak rank 3 vs 31), isolating exactly the quantity of interest: the two failures are equally far from the top at the output, yet one had a strong intermediate signal and the other never did. At full restoration strength ($\alpha = 1$), peak-state restoration recovers 12.3\% of preservation failures versus 1.8\% of their rank-matched formation counterparts (7.0$\times$; 57 matched pairs on Qwen3-8B). Because the design is paired, we report paired statistics and do not claim significance at this sample size: the paired bootstrap 95\% CI on the rate difference is $[1.8\%, 19.3\%]$ (excluding zero), the exact McNemar test gives $p = 0.070$ (7 discordant pairs favoring preservation vs 1 favoring formation; one-sided $p = 0.035$), and the unpaired Fisher test $p = 0.030$ is reported only as a reference since it ignores the pairing. The direction of the effect strengthens monotonically with restoration strength ($3.0\times$ at $\alpha = 0.8$, $6.0\times$ at $\alpha = 0.9$, $7.0\times$ at $\alpha = 1$), with the formation rate staying flat: stronger restoration helps only the group that had a peak state to restore. By contrast, an output-logit bonus applied to the same matched pairs produces essentially no separation, because the bonus exploits precisely the final rank that has been equalized. We treat this experiment as directionally consistent evidence whose paired significance is marginal at the available sample size; the activation-patching experiment below provides the statistically decisive causal test.
