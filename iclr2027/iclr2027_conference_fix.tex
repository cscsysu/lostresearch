...
We evaluate three open-weight instruction-tuned model families spanning several scales: Qwen3-14B, Qwen3-8B, Qwen3-4B, and Qwen2.5-7B for scale variation within Qwen \citep{qwen3}, plus Llama-3.1-8B \citep{dubey2024llama} and Mistral-7B-Instruct-v0.3 \citep{jiang2023mistral}. We use seven tasks: TriviaQA \citep{joshi2017triviaqa} for fact QA, HotpotQA \citep{yang2018hotpotqa} for multi-hop QA, GSM8K \citep{cobbe2021gsm8k} for math reasoning, CommonsenseQA \citep{talmor2019commonsenseqa} for commonsense reasoning, ARC-Challenge \citep{clark2018arc} for science reasoning, QuALITY \citep{pang2022quality} for long-context comprehension, and MMLU (four subjects) \citep{hendrycks2021mmlu} for world knowledge. Rank-level separation and gold-direction ablation cover all six models; the full seven-task taxonomy, teacher-forced diagnostics, and all interventions are conducted on Qwen3-8B; prediction transfer covers three model families.

\subsection{Evaluation Protocol}
All prompts use each model's chat template with the user message ``\{question\} Answer with just the answer.'', so the model is required to emit the answer directly without chain-of-thought or explanation. The analyzed \emph{answer-onset position} is the final prompt token, i.e.\ the position that predicts the first generated token; the taxonomy therefore measures whether the gold token is competitive as the first output token in the model's realized context. For each gold answer we track the first answer token, aligned by exact prompt-suffix encoding (tokenizing prompt+answer and subtracting the prompt encoding), with alias fallback to the first alias whose encoding is non-empty. Generation is greedy (temperature 0) with at most 32 new tokens. Prompts are truncated from the left to at most 512 tokens; for QuALITY the article is additionally truncated to 1{,}500 characters before prompting. MMLU uses four subjects (abstract algebra, high-school physics, professional medicine, philosophy), 50--51 examples each. Per-task evaluated counts are TriviaQA 500, HotpotQA 250, GSM8K 250, CommonsenseQA 250, ARC-Challenge 250, QuALITY 200, MMLU 200 (1{,}900 total; 1{,}494 errors on Qwen3-8B), sampled sequentially from the validation or test split named in the released configuration. All inference uses bfloat16.
...
Applying Eq.~8 to the first answer token of each error yields a small preservation-failure subset. Two decoders are involved and must not be conflated: the calibrated subset (8.9\%, bootstrap 95\% CI [6.4\%, 11.5\%]) is decoded with the \emph{tuned lens}, whereas the full seven-task error set (1{,}494 errors) was collected with the \emph{raw logit lens} and gives a strict answer-onset rate of 10.0\% (95\% CI [8.5\%, 11.6\%]): multi-hop QA 14.2\% [9.6, 20.0], world knowledge 12.6\% [8.0, 18.6], long-context comprehension 12.1\% [7.9, 17.4], knowledge QA 12.0\% [8.5, 16.4], science reasoning 10.9\% [7.2, 15.7], commonsense reasoning 9.2\% [5.5, 14.1], and mathematical reasoning 0.0\% (exact binomial upper bound 1.6\% at 0/230). The two numbers change dataset, sample size, and decoder simultaneously, so we present them as consistent magnitudes rather than as a literal replication; decoder-level stability of the label is quantified separately below. These percentages are the paper's primary taxonomy result. They concern the answer-onset token, the endpoint-conditioned strict criterion, and the named decoder; they are not sequence-level claims.

\begin{table}[t]
\centering
\caption{Primary strict answer-onset preservation rate on Qwen3-8B errors. The label is Eq.~8 under the stored zero-based $k=5$ cutoff. Intervals are exact binomial (Clopper--Pearson) 95\% CIs.}
\label{tab:strict_rates}
\begin{tabular}{lrrc}
\toprule
\textbf{Task} & \textbf{Errors} & \textbf{Rate} & \textbf{95\% CI} \\
\midrule
HotpotQA & 190 & 14.2\% & [9.6, 20.0] \\
MMLU & 167 & 12.6\% & [8.0, 18.6] \\
QuALITY & 199 & 12.1\% & [7.9, 17.4] \\
TriviaQA & 283 & 12.0\% & [8.5, 16.4] \\
ARC-Challenge & 229 & 10.9\% & [7.2, 15.7] \\
CommonsenseQA & 196 & 9.2\% & [5.5, 14.1] \\
GSM8K & 230 & 0.0\% & [0.0, 1.6] \\
\midrule
\textbf{Overall} & \textbf{1{,}494} & \textbf{10.0\%} & \textbf{[8.5, 11.6]} \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Endpoint-Free Variants and Label Robustness}
\label{sec:endpointfree}
Because Eq.~8 references the endpoint-selected competitor, we compare it against endpoint-free rank-only labels of the form $C_{\mathrm{rank}}(k_{\mathrm{in}}, k_{\mathrm{out}})=\mathbb{1}[\min_{\ell<L}r_\ell\le k_{\mathrm{in}}]\cdot\mathbb{1}[r_L>k_{\mathrm{out}}]$. Table~\ref{tab:endpointfree} reports the family alongside Eq.~8 on the same 1{,}494 errors. The closest analogue $(5,0)$ (top-band entry, final rank outside top-1) yields 21.0\% overall: higher than Eq.~8's 10.0\% because it drops the requirement that the gold token lead the eventual competitor at an intermediate layer. A stricter deep-exit hysteresis $(4,9)$ (entry into the conventional top-5, exit beyond top-10) yields only 0.7\% and measures a different, more restrictive event. Under every variant, preservation-type errors remain a minority, and the task ordering is preserved (Spearman 0.61--0.93 versus Eq.~8 across variants); sample-level overlap with Eq.~8 is moderate (Jaccard 0.48), reflecting that the competitor condition selects a subset. We therefore retain Eq.~8 as the primary competitor-relative criterion and report the endpoint-free family as the required sensitivity view; the headline conclusion (a small, task-dependent preservation regime) is unchanged under all definitions. Under the conventional top-5 cutoff ($r<5$, i.e.\ $k{=}4$) with the margin condition fixed, the Eq.~8 rate is 9.0\% versus 10.0\% at the stored $k{=}5$, so the off-by-one convention shifts the rate by about one point.

\begin{table}[t]
\centering
\caption{Endpoint-free label family versus Eq.~8 (Qwen3-8B, 1{,}494 errors, raw lens). $C_{\mathrm{rank}}(k_{\mathrm{in}},k_{\mathrm{out}})=\mathbb{1}[\min_\ell r_\ell\le k_{\mathrm{in}}]\cdot\mathbb{1}[r_L>k_{\mathrm{out}}]$. $(5,0)$ is the closest endpoint-free analogue of Eq.~8's final condition; $(4,9)$ is the deep-exit hysteresis. All variants keep preservation-type errors a minority with consistent task ordering.}
\label{tab:endpointfree}
\small
\begin{tabular}{lccccc}
\toprule
\textbf{Task} & \textbf{Eq.~8} & \textbf{$(5,0)$} & \textbf{$(4,0)$} & \textbf{$(4,9)$} & \textbf{$k{=}4$ Eq.~8} \\
\midrule
QuALITY & 12.1\% & 28.1\% & 23.6\% & 0.0\% & 11.1\% \\
TriviaQA & 12.0\% & 27.9\% & 23.3\% & 0.4\% & 10.6\% \\
HotpotQA & 14.2\% & 26.3\% & 24.2\% & 2.1\% & 13.2\% \\
ARC-Challenge & 10.9\% & 23.6\% & 21.4\% & 0.4\% & 9.2\% \\
MMLU & 12.6\% & 23.4\% & 20.4\% & 1.8\% & 10.8\% \\
CommonsenseQA & 9.2\% & 12.2\% & 12.2\% & 1.0\% & 9.2\% \\
GSM8K & 0.0\% & 5.2\% & 3.9\% & 0.0\% & 0.0\% \\
\midrule
\textbf{Overall} & \textbf{10.0\%} & \textbf{21.0\%} & \textbf{18.4\%} & \textbf{0.7\%} & \textbf{9.0\%} \\
\midrule
\multicolumn{6}{l}{\emph{Agreement with Eq.~8}: $(5,0)$ $\kappa$=0.215, $J$=0.48, $\rho_{\mathrm{task}}$=0.61; $(4,0)$ $\kappa$=0.195, $J$=0.48, $\rho_{\mathrm{task}}$=0.79; $k{=}4$ $\kappa$=0.195, $J$=0.90, $\rho_{\mathrm{task}}$=0.93.} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Category counts.}
The four taxonomy categories are mutually exclusive and exhaustive on the evaluated set: among 1{,}494 errors, 1{,}339 (89.6\%) are formation failures, 149 (10.0\%) are preservation failures (Eq.~8 positive), and 6 (0.4\%) are unresolved, of which 5 have the gold token as the final top-1 at the answer-onset position yet a judged-wrong full answer, i.e.\ the error arises downstream of the first token. Among the 406 correct examples, 351 (86.5\%) show an intermediate top-band gold rank (\emph{successful preservation}) and 55 (13.5\%) emerge only late.
...
\paragraph{Gold-free prediction within known errors (exploratory).}
\label{sec:nonoracle}
The preceding predictors require the gold token. We also train a random forest using only the model's first-half five-token candidate distributions (leader churn, prefix-leader hold, top-1/top-2 margins, five-token entropy). In a pooled out-of-fold evaluation over \emph{known errors}, this gold-free model reaches ROC-AUC 0.669 (bootstrap 95\% CI [0.630, 0.709]) and PR-AUC 0.383 at a 0.294 base rate, with a 1.35$\times$ precision lift at the top 20\%. This is an exploratory, error-subtyping analysis: it assumes an upstream mechanism has already flagged the answer as erroneous and requires calibration data from the target task distribution; it is not an end-to-end deployment result.
...
\paragraph{Effect-matched controls.}
A stronger circularity concern is that the patch recovers examples merely because it raises the gold logit, in which case any perturbation producing the same instantaneous logit gain should recover equally. We attempted to construct such controls by scaling random directions orthogonal to $\hat w_{y^\star}$, and directions of frequent non-gold tokens, until the patched final gold logit matches the gold patch's gain. For the 17 preservation failures with the largest measurable gains, \emph{no} non-gold direction could reproduce the gain even at perturbation norms $25.7\times$ larger than the gold patch (0/17 matched within the search cap): adding orthogonal mass dilutes the gold logit under the final RMSNorm rather than raising it, and these controls recover 0/17 examples. The attempted matching therefore establishes a stronger fact than a matched control would: within this perturbation family, the instantaneous gold-logit increase is itself direction-specific and cannot be produced by generic perturbation magnitude. Together with the rank-matched finding that a pure output-level logit bonus produces no group separation while internal restoration does, the recovery effect cannot be reduced to logit inflation by an arbitrary intervention.
...
where $\hat d$ is the unit gold-versus-competitor direction. On preservation failures, the layer-35 MLP patch achieves \emph{gold-token top-1 recovery} (single forward pass, answer-onset position) of 11.0\%, 20.2\%, and 31.2\% at $\gamma=1,2,4$, while an orthogonal random direction recovers 0/109. Adjacent MLPs recover similarly (layer 34: 12.8--33.0\%; layer 33: 11.9--25.7\%; layer 31: 4.6--22.0\%), and full zero-ablation performs worse. These results show that late MLP writes contain margin-opposing components whose targeted removal can increase gold-token top-1 recovery, and that this opportunity is distributed across layers 31--35. Because the target direction is explicitly constructed from the gold and competitor unembeddings, the experiment does not prove that the network contains a uniquely identifiable semantic ``correct-answer variable'' or that the intervention can be selected without oracle information.
