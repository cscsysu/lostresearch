...
To control the final-rank confound directly, we match each preservation failure to a formation failure with the same final gold rank (within $\pm 3$) and apply the peak-state restoration above to both. Even with final rank equalized across the two groups, preservation failures recover substantially more than their rank-matched formation counterparts, whereas the earlier output-logit bonus applied to the same matched pairs produced no separation (the bonus exploits precisely the quantity being matched). This isolates the intermediate peak state as the source of repairability.
