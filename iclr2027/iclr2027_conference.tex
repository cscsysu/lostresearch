...
To control the final-rank confound directly, we match each preservation failure to a \emph{true} formation failure (peak gold rank $> 20$, so it never carried a strong intermediate signal) with the same final gold rank (within $\pm 3$), and apply the peak-state restoration above to both. The matched groups have nearly identical final-rank distributions (median 8 for preservation vs 11 for formation) but sharply different peak states (median peak rank 3 vs 31), isolating exactly the quantity of interest: the two failures are equally far from the top at the output, yet one had a strong intermediate signal and the other never did. Under this control, peak-state restoration recovers 10.5\% of preservation failures versus 3.5\% of their rank-matched formation counterparts (3.0$\times$; 57 matched pairs on Qwen3-8B). By contrast, an output-logit bonus applied to the same matched pairs produces essentially no separation, because the bonus exploits precisely the final rank that has been equalized. The 3.0$\times$ gap under matched final rank isolates the intermediate peak state, not the output-layer distance, as the source of differential repairability.
