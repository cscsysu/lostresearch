...
Applying our strict criterion (Eq.~8: rank $\le 5$, $\mathrm{CIS}^{\mathrm{comp}}_\ell > 0$, and final $\mathrm{CIS}^{\mathrm{comp}}_L < 0$) to error examples reveals that preservation failures under this definition are a small minority. On the Qwen3-8B calibrated analysis subset, the tuned lens identifies 8.9\% (bootstrap 95\% CI [6.4\%, 11.5\%]) under the strict criterion. We additionally report a broader \emph{signal-loss} metric in Table~\ref{tab:taskdecay}: any answer token entering top-5 at an intermediate layer but dropping below rank 10 at the final layer, which yields higher rates (5--28\% across tasks). The two metrics capture different aspects: the strict criterion (Eq.~8) requires the gold signal to lead the eventual competitor, while the signal-loss metric only requires intermediate competitiveness followed by final-layer decline. Both confirm that preservation-type failures are task-dependent and concentrated in knowledge-intensive tasks, though they differ in absolute rates.
...
As detailed in Table~\ref{tab:taskdecay}, preservation failures are highly task-dependent, spanning a range from 5\% to 28\% across seven tasks under multi-token tracking. Knowledge QA (TriviaQA, 27.9\%), long-context comprehension (QuALITY, 27.6\%), and multi-hop reasoning (HotpotQA, 26.3\%) show the highest rates. Science reasoning (ARC-Challenge, 23.6\%) and world knowledge (MMLU, 23.4\%) are slightly lower. Commonsense reasoning (CommonsenseQA) is markedly lower at 12.2\%, and mathematical reasoning (GSM8K) is lowest at 5.2\%. This gradient reveals a clear pattern: tasks requiring factual retrieval or long-context integration exhibit the highest preservation-failure rates, while tasks requiring multi-step procedural computation exhibit the lowest, consistent with our interpretation that preservation failures arise when the model forms a correct factual association but fails to maintain it through late-layer computation.

\begin{table}[t]
\centering
\caption{Signal-loss rate (preservation failure) by task on Qwen3-8B with multi-token tracking, measured as the fraction of errors where any gold-answer token enters top-5 at some intermediate layer but drops below rank 10 at the final layer. Tasks are ordered by signal-loss rate.}
\label{tab:taskdecay}
\begin{tabular}{lccc}
\toprule
...
The predictors above use the gold token to compute rank and margin trajectories, which is available offline but not at deployment time. To show that decay is predictable from quantities observable during ordinary generation, we train a predictor that uses \emph{only} the model's own top-5 token distributions at each layer, without ever referencing the gold answer. From these distributions we derive layer-wise features that describe the competitive dynamics among the model's own leading candidates: how often the top-1 token switches identity across layers (leader churn), the fraction of layers the eventual leader holds, the margin between the first and second candidates, and the entropy of the top-5 mass. A random forest on these features predicts final-layer decay with ROC-AUC 0.670 (PR-AUC 0.385), with leader churn (the mean rate at which the top candidate changes across layers) as the most informative feature. This is a legitimate deployable trigger: it observes only what the model itself exposes at inference time, yet still recovers most of the endpoint-free signal, showing that the instability characteristic of preservation failures is visible in the model's own candidate dynamics before the gold identity is known.
...
The final-layer MLP (layer 35 for Qwen3-8B) has the largest negative direct attribution among all layers. We report this as an attribution; we validate its causal role with the activation-patching experiment in Section~\ref{sec:causal-patch}, which shows that restoring the gold-direction component removed by the late-layer computation recovers the gold answer for preservation failures. Figure~\ref{fig:intervention}b shows the per-layer attribution.
