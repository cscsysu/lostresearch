...
The final-layer MLP (layer 35 for Qwen3-8B) has the largest negative direct attribution among all layers. We report this as an attribution. The peak-state restoration experiment in Section~\ref{sec:causal-patch} establishes causally that restoring the decayed gold-direction component recovers the gold answer for preservation failures; which \emph{specific component} removed that component is validated with component-selective patching of the layer-35 MLP in the same section. Figure~\ref{fig:intervention}b shows the per-layer attribution.
...
\paragraph{Closing the loop with the non-oracle trigger.}
The 127\% figure above uses an oracle-assisted trigger. Replacing it with the strictly non-oracle, leak-audited predictor of Section~\ref{sec:nonoracle} as the trigger (out-of-fold probabilities, top-30\% coverage) yields 3.3\% recovery per intervention versus 3.0\% for indiscriminate steering at the same steering strength -- an 11\% efficiency gain while saving 70\% of the intervention budget, and the same ground-truth differential under steering (5.6\% preservation vs 0.9\% formation, 6.2$\times$). Comparing the two operating points quantifies the value of the oracle information: removing it preserves the budget saving and the differential-repairability signature but shrinks the efficiency gain from 127\% to 11\%, because the non-oracle trigger recovers only about a quarter of the true preservation positives at this coverage (21/89). We report this gap explicitly rather than extrapolating the oracle-assisted number to deployment.
...
\paragraph{Component-selective patching of the layer-35 MLP.}
The peak-state patch above restores the signal at every layer after $L^\star$ and therefore does not, by itself, identify \emph{which component} removed it. To test the layer-35 MLP attribution directly, we patch a \emph{single component at a single layer}: at the MLP output of layer $\ell$, we remove exactly the component of the MLP's write that points \emph{against} the gold direction, $m' = m - \gamma\,\mathrm{clamp}_{\le 0}(m^\top \hat{w}_{y^\star})\,\hat{w}_{y^\star}$ (with $\gamma{=}1$ the gold-suppressive part of this MLP's contribution is deleted, everything else untouched). We apply the identical patch to: the layer-35 MLP (the attributed component), the MLPs of adjacent layers (33, 34, 36, 37), the layer-35 attention output, and a control that removes a random orthogonal direction from the layer-35 MLP. If the attribution is causal and specific, deleting the gold-suppressive write of the layer-35 MLP alone should improve the gold rank substantially more than any of these controls. We report the net recovery-to-top-1 rate and the mean gold-rank change per condition.
