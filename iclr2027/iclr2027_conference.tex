...
The layer-35 MLP attribution in Section~\ref{sec:components} is correlational. To test it causally, we patch, at every layer after $L^\star$, the gold-direction component of the residual that decayed relative to the peak state, $h_\ell' = h_\ell + \beta\,\mathrm{clamp}_{\ge 0}\!\big((h_{L^\star}-h_\ell)^\top \hat{w}_{y^\star}\big)\,\hat{w}_{y^\star}$, where $\hat{w}_{y^\star}$ is the unit unembedding direction of the gold token. Restoring this specific decayed component recovers the gold answer far more often for preservation failures than for (i) formation failures under the identical patch and (ii) a matched-null control that patches a random competitor's direction with the same strength. The patch thus causally reinstates the gold signal that the late-layer computation had removed, validating the attribution as more than a correlational ranking.
