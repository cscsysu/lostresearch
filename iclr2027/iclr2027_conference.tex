...
\end{equation}
All answer-level analyses (Table~\ref{tab:taskdecay}, signal-loss rates) use $r^{\mathrm{multi}}_\ell$ in place of $r_\ell(y^\star)$. This ensures that multi-word entities (e.g., ``New York'') and multi-digit numbers are detected as competitive if \emph{any} constituent token enters the top-$k$ region. The competitive-margin trajectory $\mathrm{CIS}^{\mathrm{comp}}_\ell$ and the strict competitive-decay label (Eq.~8, used in Figure~\ref{fig:taxonomy}) continue to use the first answer token $y^\star_1$, which provides the most conservative estimate. We verified that using $r^{\mathrm{multi}}_\ell$ versus $r_\ell(y^\star_1)$ produces nearly identical signal-loss rates on tasks with predominantly single-token answers (TriviaQA, CommonsenseQA, GSM8K) and only modest differences on multi-word tasks (HotpotQA), confirming that multi-token tracking does not introduce cherry-picking artifacts.
