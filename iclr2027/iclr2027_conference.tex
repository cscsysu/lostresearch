...
This rate is not a probe artifact: the tuned-lens translators are trained on unlabelled prompts by predicting the model's own final-layer logits, without observing ground-truth answers or correctness labels, so no downstream task information leaks into intermediate readouts.
...
Because the competitive margin $\mathrm{CIS}^{\mathrm{comp}}_\ell$ references the final-layer competitor $y^-$, we also trained an endpoint-free variant using only features that depend on the gold token itself (its log-probability, rank, and their slopes). This endpoint-free predictor reaches AUC 0.66, above all single-layer baselines and temporal-extrapolation baselines (persistence 0.60, AR 0.62), confirming that the gold-answer trajectory alone carries predictive signal about future decay; the full trajectory's 0.75 adds the gold-vs-competitor contrast, a legitimate trajectory quantity.
