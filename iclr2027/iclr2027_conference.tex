...
Applying our strict criterion (Eq.~8: rank $\le 5$, $\mathrm{CIS}^{\mathrm{comp}}_\ell > 0$, and final $\mathrm{CIS}^{\mathrm{comp}}_L < 0$) to error examples reveals that preservation failures under this definition are a small minority. On the Qwen3-8B calibrated analysis subset, the tuned lens identifies 8.9\% (bootstrap 95\% CI [6.4\%, 11.5\%]) under the strict criterion. To keep the strict and permissive metrics strictly separated, Table~\ref{tab:taskdecay} now reports \emph{both} per task: the strict teacher-forced ALL-based sequence-level rate as the main metric and the broader signal-loss rate (any answer token entering top-5 then falling below rank 10) only as a permissive upper bound. The two metrics capture different aspects: the strict criterion (Eq.~8) requires the gold signal to lead the eventual competitor, while the signal-loss metric only requires intermediate competitiveness followed by final-layer decline. Both show that preservation-type failures are task-dependent and concentrated in knowledge-intensive tasks, but we base all conclusions on the strict criterion and use the permissive rates solely as a sensitivity bound.
% TODO(server): insert strict per-task rates with CIs from
% `python run_strict_task_rates.py` (7-task full_results) into Table 3.
