...
\paragraph{Task dependence: Preservation failures concentrate in knowledge and science tasks.}
As detailed in Table~\ref{tab:taskdecay} and Figure~\ref{fig:taxonomy}, preservation failures are highly task-dependent. On single-hop factual recall (TriviaQA) and multi-hop entity QA (HotpotQA), the signal-loss rate (gold entering top-5 at an intermediate layer but falling below rank 10 at the final layer) reaches 28.5\% and 25.3\% respectively. Science reasoning (ARC-Challenge) shows a similar rate of 23.6\%, while commonsense reasoning (CommonsenseQA) is lower at 11.8\%, and mathematical reasoning (GSM8K) is lowest at 5.2\%. This gradient from knowledge-intensive tasks (highest) through reasoning tasks (intermediate) to mathematical computation (lowest) suggests that preservation failures are most prevalent when the model possesses the relevant factual association but fails to maintain it through late-layer computation.

\begin{table}[t]
\centering
\caption{Signal-loss rate (preservation failure) by task on Qwen3-8B. Measured as the fraction of errors where the gold token enters top-5 at some intermediate layer but drops below rank 10 at the final layer.}
\label{tab:taskdecay}
\begin{tabular}{lccc}
\toprule
