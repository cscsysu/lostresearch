...
The minimum-over-tokens rule is permissive, so we validate the taxonomy under a stricter teacher-forced, sequence-level protocol. We append the gold answer to the prompt (teacher forcing) and, for each answer token, read the hidden state at the position that predicts it, decoding the gold rank and log-probability at every layer. We then aggregate across answer positions with three well-defined rules applied consistently at every layer: \textbf{ALL} (every answer token is top-$k$ at the same intermediate layer), \textbf{MEAN} (the mean per-token rank is top-$k$), and \textbf{JOINT} (the intermediate sequence log-likelihood exceeds the final one). On 300 Qwen3-8B errors, the sequence-level preservation rate is 22.7\% (ALL), 39.7\% (MEAN), and 70.7\% (JOINT), whereas the permissive minimum-over-tokens rule gives 92.7\%. The strict ALL rate (22.7\%) is of the same order as our main signal-loss estimates (5--28\%), while the inflated 92.7\% under the minimum rule confirms that ``any token ever entering top-$k$'' is a cherry-picking artifact: it grows with answer length (46.4\% for single-token answers vs 18.8\% for answers of three or more tokens under ALL), exactly the bias a single-token or minimum criterion would introduce. We therefore report the conservative first-token and ALL-based rates throughout and treat the minimum rule only as a permissive upper bound.
...
Table~\ref{tab:taskdecay} reports the per-task preservation rate under the strict teacher-forced sequence-level criterion (ALL: every gold-answer token enters the top-$k$ at the same intermediate layer under teacher forcing; Section 3.3), which is our main answer-level metric. A permissive sensitivity upper bound based on the minimum-over-tokens rule is reported alongside for comparison; as shown in Section 3.3, the minimum rule inflates rates with answer length and cannot carry the main conclusion. Under the strict criterion, knowledge QA, long-context comprehension, and multi-hop reasoning show the highest rates, while mathematical reasoning is lowest. This gradient is consistent with our interpretation that preservation failures arise when the model forms a correct factual association but fails to maintain it through late-layer computation, whereas procedural computation either succeeds outright or never forms the final-answer signal.
% TODO(server): replace Table 3 numbers with strict ALL-based per-task rates from
% `python run_teacher_forced_multitoken.py --n 1600`, keep min-rule column as
% sensitivity upper bound.

\begin{table}[t]
\centering
\caption{Preservation rate by task on Qwen3-8B. Main metric: strict teacher-forced sequence-level criterion (ALL — every gold-answer token top-$k$ at the same intermediate layer). Sensitivity upper bound: permissive min-over-tokens rule (any gold token ever entering top-5 then falling below rank 10), which inflates rates with answer length (Section 3.3) and is reported only as an upper bound. Tasks ordered by the strict rate.}
\label{tab:taskdecay}
\begin{tabular}{lccc}
\toprule
\textbf{Task} & \textbf{Type} & \textbf{Total Errors} & \textbf{Signal-Loss Rate} \\
\midrule
TriviaQA & Knowledge QA & 283 & 27.9\% \\
QuALITY & Long-context comprehension & 199 & 27.6\% \\
HotpotQA & Multi-hop QA & 190 & 26.3\% \\
ARC-Challenge & Science reasoning & 229 & 23.6\% \\
MMLU & World knowledge (multi-subject) & 167 & 23.4\% \\
CommonsenseQA & Commonsense reasoning & 196 & 12.2\% \\
GSM8K & Math reasoning & 230 & 5.2\% \\
\bottomrule
\end{tabular}
\end{table}
