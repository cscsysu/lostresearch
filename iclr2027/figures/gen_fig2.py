"""
Figure 2: Measurement bias.
Left: observed vs nulls crossing rate — ALL NUMBERS READ FROM matched_null JSON
      (single source of truth; no hardcoded values).
Right: raw vs tuned peak CIS.
Generates: fig2_measurement_bias.pdf / .png
"""
import json
import os
import numpy as np
import matplotlib.pyplot as plt

plt.rcParams.update({
    "font.size": 11,
    "axes.titlesize": 12,
    "axes.labelsize": 11,
    "xtick.labelsize": 10.5,
    "ytick.labelsize": 10.5,
    "legend.fontsize": 10,
})

# === Single source of truth: matched-null results ===
HERE = os.path.dirname(os.path.abspath(__file__))
CANDIDATES = [
    os.path.join(HERE, "..", "..", "lostresearch", "outputs", "data",
                 "matched_null_Qwen3-8B.json"),
    os.path.join(HERE, "..", "..", "lost-output", "outputs", "data",
                 "matched_null_Qwen3-8B.json"),
]
mn = None
for c in CANDIDATES:
    if os.path.exists(c):
        with open(c) as f:
            mn = json.load(f)
        print(f"Reading matched-null results from: {c}")
        break
if mn is None:
    raise FileNotFoundError("matched_null_Qwen3-8B.json not found")

OBSERVED = 100 * mn["observed_rate"]                       # 92.2
RANDOM_NULL = 100 * mn["random_null_mean"]                 # 98.8
RANK_NULL = 100 * mn["matched_null_mean"]                  # 98.7
print(f"observed={OBSERVED:.1f}  random_null={RANDOM_NULL:.1f}  "
      f"rank_matched_null={RANK_NULL:.1f}")

fig, axes = plt.subplots(1, 2, figsize=(11.5, 4.2))
fig.subplots_adjust(left=0.08, right=0.96, top=0.88, bottom=0.18, wspace=0.28)

# === Left: observed vs nulls crossing rate ===
ax = axes[0]
labels = ["Observed\n(gold vs final\ncompetitor)", "Random-\ncompetitor null",
          "Rank-matched\nnull"]
rates = [OBSERVED, RANDOM_NULL, RANK_NULL]
colors = ["#C0392B", "#5DADE2", "#8E44AD"]
bars = ax.bar(labels, rates, color=colors, edgecolor="black", linewidth=0.8,
              width=0.55)
ax.set_ylim(0, 115)
ax.set_ylabel("Crossing rate (%)")
ax.set_title("(a) Pairwise crossing is trivially weak", loc="left",
             fontweight="bold")
ax.set_yticks([0, 25, 50, 75, 100])
ax.grid(axis="y", alpha=0.2)

for bar, r in zip(bars, rates):
    ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 2,
            f"{r:.1f}%", ha="center", va="bottom", fontsize=10.5,
            fontweight="bold")

ax.text(0.5, 0.30, "Both nulls exceed the observed rate.\n"
        "Almost any token is crossed: no evidence\nof a competitive signal.",
        transform=ax.transAxes, ha="center", va="center",
        fontsize=9, style="italic", color="#444444",
        bbox=dict(boxstyle="round,pad=0.4", facecolor="#FEF9E7",
                  edgecolor="#F4D03F", alpha=0.95))

# === Right: raw vs tuned peak CIS ===
ax = axes[1]
models = ["Qwen3-8B", "Llama-3.1-8B", "Mistral-7B"]
raw_vals = [4.53, 4.6, 4.4]
tuned_vals = [2.24, 2.3, 2.2]
x = np.arange(len(models))
width = 0.32

bars1 = ax.bar(x - width / 2, raw_vals, width, color="#E67E22",
               edgecolor="black", linewidth=0.8, label="Raw logit lens")
bars2 = ax.bar(x + width / 2, tuned_vals, width, color="#27AE60",
               edgecolor="black", linewidth=0.8, label="Tuned lens")

ax.set_ylabel("Peak intermediate CIS")
ax.set_title("(b) Raw vs tuned peak CIS (incorrect examples)",
             loc="left", fontweight="bold")
ax.set_xticks(x)
ax.set_xticklabels(models)
ax.set_ylim(0, 6.8)
ax.legend(loc="upper right", framealpha=0.95, fontsize=9.5)
ax.grid(axis="y", alpha=0.2)

for bar, v in zip(bars1, raw_vals):
    ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.1,
            f"{v:.1f}", ha="center", va="bottom", fontsize=10)
for bar, v in zip(bars2, tuned_vals):
    ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.1,
            f"{v:.1f}", ha="center", va="bottom", fontsize=10)

ax.annotate("Raw inflates by ~2.0x",
            xy=(-0.16, 4.68), xytext=(0.55, 5.75),
            fontsize=9.5, color="#C0392B", fontweight="bold",
            arrowprops=dict(arrowstyle="->", color="#C0392B", lw=1.2,
                            connectionstyle="arc3,rad=-0.18"),
            ha="center", va="center")

plt.savefig(os.path.join(HERE, "fig2_measurement_bias.pdf"), dpi=200)
plt.savefig(os.path.join(HERE, "fig2_measurement_bias.png"), dpi=200)
plt.close()
print("Figure 2 generated from matched_null JSON.")
